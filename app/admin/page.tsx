...
export default function AdminPage() {
  ...
  const [bonusForm, setBonusForm] = useState({ title: "", amount: 5 });
  ...
  const loadData = async () => {
    ...
    const pageSize = 1000;
    while (true) {
      const { data: page } = await supabase.from("users").select("*").range(from, from + pageSize - 1);
  ...
  const handleImportCSV = async () => {
    if (!importFile) { setMessage("Seleziona un file CSV o Excel"); return; }
  ...
  const handleCreateEvent = async () => {
    ...
    if (error) { setMessage("Errore creazione evento: " + error.message); }
    else {
  ...
  const handleCreateBonus = async () => {
    ...
    const { error } = await supabase.from("bonus_qr").insert({ title: bonusForm.title, amount: bonusForm.amount, code });
    if (error) { setMessage("Errore creazione bonus: " + error.message); }
    else {
  ...
  const handleReset = async () => {
    let confirmMsg = "";
    if (resetType === "today") confirmMsg = "🗑️ Vuoi cancellare SOLO i voti di oggi? (I voti storici rimarranno)";
    else if (resetType === "scores") confirmMsg = "⚠️ Vuoi cancellare TUTTI i voti (storici + oggi)? Questa operazione è irreversibile!";
    else if (resetType === "full") confirmMsg = "🚨 RESET COMPLETO: Cancellerai TUTTO (voti, eventi, bonus). Sei ASSOLUTAMENTE sicuro?";
  ...
  const handleAddAdmin = async () => {
    ...
    if (error) { setMessage("Errore nomina admin: " + error.message); }
    else { setMessage("✅ Admin nominato!"); setShowAdminModal(false); setAdminEmail(""); loadData(); }
  };

  if (loading) return <div style={{ textAlign: "center", padding: 40 }}>Verifica credenziali...</div>;
  if (!isAdmin) return <div style={{ textAlign: "center", padding: 40 }}>Accesso negato. Non hai i permessi per visualizzare questa pagina.</div>;

  // Tendina team riutilizzabile con tutte e 4 le opzioni
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <button onClick={() => router.push("/")} style={{ color: "#FF6B35", background: "none", border: "none", fontSize: "1rem", cursor: "pointer" }}>
          ← Torna alla dashboard
        </button>
      </div>

      {message && <div style={{ padding: 12, background: "#f0f0f0", borderRadius: 8, marginBottom: 16 }}>{message}</div>}

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginTop: 20 }}>
        <button onClick={() => setShowEventModal(true)} style={{ padding: 12, background: "#1E3A5F", color: "white", border: "none", borderRadius: 8, cursor: "pointer" }}>🎤 Crea Evento</button>
        <button onClick={() => setShowBonusModal(true)} style={{ padding: 12, background: "#FF6B35", color: "white", border: "none", borderRadius: 8, cursor: "pointer" }}>🎁 Crea QR Bonus</button>
      ...
      <div style={{ marginTop: 20 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 8 }}>
          <thead><tr style={{ borderBottom: "2px solid #ddd" }}><th style={{ textAlign: "left", padding: 8 }}>Titolo</th><th style={{ textAlign: "left", padding: 8 }}>Target</th><th style={{ textAlign: "left", padding: 8 }}>QR</th></tr></thead>
          <tbody>{events.map((e) => (<tr key={e.id} style={{ borderBottom: "1px solid #eee" }}><td style={{ padding: 8 }}>{e.title}</td><td style={{ padding: 8 }}>{e.team_target}</td><td style={{ padding: 8, fontSize: "0.8rem" }}>{e.qr_code}</td></tr>))}</tbody>
        </table>
      </div>
      ...
      <div style={{ marginTop: 20 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 8 }}>
          <thead><tr style={{ borderBottom: "2px solid #ddd" }}><th style={{ textAlign: "left", padding: 8 }}>Titolo</th><th style={{ textAlign: "left", padding: 8 }}>Importo</th><th style={{ textAlign: "left", padding: 8 }}>Codice</th></tr></thead>
          <tbody>{bonuses.map((b) => (<tr key={b.id} style={{ borderBottom: "1px solid #eee" }}><td style={{ padding: 8 }}>{b.title}</td><td style={{ padding: 8 }}>+{b.amount}</td><td style={{ padding: 8, fontSize: "0.8rem" }}>{b.code}</td></tr>))}</tbody>
        </table>
      </div>
      ...
      <div style={{ marginTop: 20 }}>
        ...
        <div style={{ maxHeight: 500, overflowY: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 8 }}>
            <thead><tr style={{ borderBottom: "2px solid #ddd" }}><th style={{ textAlign: "left", padding: 8 }}>Nome</th><th style={{ textAlign: "left", padding: 8 }}>Email</th><th style={{ textAlign: "left", padding: 8 }}>Team</th><th style={{ textAlign: "left", padding: 8 }}>Ruolo</th><th style={{ textAlign: "center", padding: 8, minWidth: 80 }}>Azioni</th></tr></thead>
            <tbody>
              {filteredUsers.map((u) => {
                ...
                return (
                  <tr key={u.id} style={{ borderBottom: "1px solid #eee" }}>
                    ...
                    <td style={{ padding: 8 }}>
                      <span style={{ padding: "2px 8px", borderRadius: 4, background: u.role === "admin" ? "#dc3545" : u.role === "staff" ? "#6f42c1" : "#28a745", color: "white", fontSize: "0.7rem", fontWeight: "bold" }}>{u.role}</span>
                    </td>
                    <td style={{ textAlign: "center", padding: 8 }}>
                      <button onClick={() => openEditModal(u)} style={{ padding: "4px 8px", marginRight: 4, background: "#ffc107", border: "none", borderRadius: 4, cursor: "pointer", fontSize: "0.8rem" }} title="Modifica utente">✏️</button>
                      <button onClick={() => handleDeleteUser(u.id, `${u.first_name} ${u.last_name}`)} style={{ padding: "4px 8px", background: isProtected ? "#ccc" : "#dc3545", color: "white", border: "none", borderRadius: 4, cursor: isProtected ? "not-allowed" : "pointer", fontSize: "0.8rem", opacity: isProtected ? 0.5 : 1 }} disabled={isProtected} title={isProtected ? "Non puoi eliminare l'admin principale" : "Elimina utente"}>🗑️</button>
                    </td>
                  </tr>
                );
              })}
      ...
      {showEditUserModal && selectedUser && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "white", padding: 24, borderRadius: 16, maxWidth: 500, width: "100%" }}>
            ...
            <p style={{ color: "#999", fontSize: "0.8rem", marginBottom: 8 }}>Modifica i dati di {selectedUser.first_name} {selectedUser.last_name}</p>
            <input type="email" placeholder="Email" value={userForm.email} onChange={(e) => setUserForm({ ...userForm, email: e.target.value })} style={{ width: "100%", padding: 8, marginTop: 8 }} />
            <input type="text" placeholder="Nome" value={userForm.first_name} onChange={(e) => setUserForm({ ...userForm, first_name: e.target.value })} style={{ width: "100%", padding: 8, marginTop: 8 }} />
            <input type="text" placeholder="Cognome" value={userForm.last_name} onChange={(e) => setUserForm({ ...userForm, last_name: e.target.value })} style={{ width: "100%", padding: 8, marginTop: 8 }} />
            <TeamSelect value={userForm.team} onChange={(v) => setUserForm({ ...userForm, team: v })} />
